...
void tx_data(const char data) { // transmitir dados
    //O sinalizador TXIF � ativado quando TX REG est� vazio
...
unsigned char rx_data(void) { // receber dados
    while (!RCIF); // Wait until RCIF gets low - Espere at� RCIF ficar baixo
    return RCREG; // Retrieve data from reception register - Recuperar dados do registro de recep��o  
}
...
void realizar_alteracoes(){
    switch(dado[1]){
        ...
        case ALARME:
            alarme.hora = dado[2];
            alarme.minuto = dado[3];
            alarme.segundo = dado[4];
        break;                 
    }   
}
